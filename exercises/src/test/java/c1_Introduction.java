...
public class c1_Introduction extends IntroductionBase {
    ...
    public void hello_world() {
        ...
        //метод block блокирует текущий поток, пока mono не завершится и, если mono завершается - возвращает результат
    ...
    public void unresponsive_service() {
        Exception exception = assertThrows(IllegalStateException.class, () -> {
            ...
            //аналогично block, но мы устанавливаем ограничение по времени, в которое должен быть завершен mono
            //если mono выполняется большее время, то получаем exception с текстовкой о тайм-ауте
    ...
    public void empty_service() {
        ...
        //извлекаем результат выполнения mono в Optional
    ...
    public void multi_result_service() {
        ...
        //возьмем из flux только первое значение
    ...
    public void nothing_happens_until_you_() throws InterruptedException {
        ...
        //subscribe(): Этот метод инициирует выполнение Flux.
        //Без подписки Flux не будет эмитировать значения, и, следовательно, doOnNext(companyList::add) не будет вызван.
        //Подписка на Flux позволяет ему начать эмитировать значения, которые затем будут добавлены в companyList.
        serviceResult
    ...
    public void leaving_blocking_world_behind() throws InterruptedException {
        ...
        fortuneTop5()
                .doOnNext(companyList::add) // добавляем компании в список
                .subscribe(
                        null, // обработчик ошибок (можно оставить null, если не нужно обрабатывать)
                        null, // обработчик ошибок (можно оставить null, если не нужно обрабатывать)
                        () -> serviceCallCompleted.set(true) // устанавливаем флаг завершения
                );
